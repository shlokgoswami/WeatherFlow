...
def save_joined_data_s3(task_instance):
    ...
    df.to_csv(f"s3://airflow-parallel/{dt_string}.csv", index=False)
...
default_args = {
    ...
    'email': ['shlokgoswami5@gmail.com@gmail.com'],
...
with DAG('weather_dag_2',
        ...
        #Task group for locally uploaded data into s3- (We create table in postgres and the load the s3 bucket data into that table)
        with TaskGroup(group_id = 'group_a', tooltip= "Extract_from_S3_and_weatherapi") as group_A:
            ...
            uploadS3_to_postgres  = PostgresOperator(
                ...
                postgres_conn_id = "postgres_conn",
                sql = "SELECT aws_s3.table_import_from_s3('city_look_up', '', '(format csv, DELIMITER '','', HEADER true)', 'airflow-parallel', 'us_city.csv', 'us-east-1');"
            )
            ...
            is_houston_weather_api_ready = HttpSensor(
                ...
                endpoint='/data/2.5/weather?q=houston&APPID=6334106f9156ecd088b04b284c7758e6'
            )
